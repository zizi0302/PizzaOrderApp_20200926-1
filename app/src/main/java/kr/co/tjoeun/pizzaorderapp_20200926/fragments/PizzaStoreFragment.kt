...
//        fragment_pizza_store xml을 그려서 -> 프래그먼트의 화면으로 (결과) 지정
...
//        리스트뷰의 어댑터로서 => mStoreAdapter를 지정, pizzaStorelistView의 내용물을 관리하게 된다.
        pizzaStoreListView.adapter = mStoreAdapter

//  => 여기까지가 리스트뷰에 내용물을 띄우기 위한 코드
        
//        리스트뷰 아이템이 눌렸을 때 (피자 가게 하나가 눌렸을때) 이벤트 처리.
...
//            상세 조회 화면으로 (가게데이터를 통째로 들고) 이용
            val myIntent = Intent(context, ViewStoreDetailActivity::class.java)
//            Store클래스에 Serializable 속성을 부여했기 때문에 통쨰로 첨부 가능
